...
class TestEuler < Minitest::Test
  EULER_ZERO = OpenGL::Euler.new(0, 0, 0, "XYZ")
  EULER_A_XYZ = OpenGL::Euler.new(1, 0, 0, "XYZ")
  EULER_A_ZYX = OpenGL::Euler.new(0, 1, 0, "ZYX")
  ...
  def test_set_set_from_vector3_to_vector3
    ...
    vec = OpenGL::Vector3.new(0, 1, 0)

    b = OpenGL::Euler.new.set_from_vector3(vec, "ZYX")
    # console.log(a, b)
    assert_equal(a, b)
  ...
  def test_quaternion_set_from_euler_euler_from_quaternion
    [EULER_ZERO, EULER_A_XYZ, EULER_A_ZYX].map(&.clone).each do |v|
      q = OpenGL::Quaternion.new.set_from_euler(v)

      v2 = OpenGL::Euler.new.set_from_quaternion(q, v.order)
      q2 = OpenGL::Quaternion.new.set_from_euler(v2)
  ...
  def test_matrix4_set_from_euler_euler_from_rotation_matrix
    [EULER_ZERO, EULER_A_XYZ, EULER_A_ZYX].map(&.clone).each do |v|
      m = OpenGL::Matrix4.new.make_rotation_from_euler(v)

      v2 = OpenGL::Euler.new.set_from_rotation_matrix(m, v.order)
      m2 = OpenGL::Matrix4.new.make_rotation_from_euler(v2)
  ...
  def test_reorder
    [EULER_ZERO, EULER_A_XYZ, EULER_A_ZYX].map(&.clone).each do |v|
      ...
      v.reorder("YZX")
      q2 = OpenGL::Quaternion.new.set_from_euler(v)
      assert_quat_equals(q2, q)

      v.reorder("ZXY")
      q3 = OpenGL::Quaternion.new.set_from_euler(v)
  ...
  def test_gimbal_local_quat
    ...
    q1 = OpenGL::Quaternion.new(0.5207769385244341, -0.4783214164122354, 0.520776938524434, 0.47832141641223547)
    # q2 = OpenGL::Quaternion.new(0.11284905712620674, 0.6980437630368944, -0.11284905712620674, 0.6980437630368944)

    euler_order = "ZYX"

    # create Euler directly from a Quaternion
    e_via_q1 = OpenGL::Euler.new.set_from_quaternion(q1, euler_order) # there is likely a bug here

    # create Euler from Quaternion via an intermediate Matrix4
    m_via_q1 = OpenGL::Matrix4.new.make_rotation_from_quaternion(q1)
    e_via_m_via_q1 = OpenGL::Euler.new.set_from_rotation_matrix(m_via_q1, euler_order)

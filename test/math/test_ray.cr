...
class TestRay < Minitest::Test
  ...
  def test_constructor_equals
    ...
    a = OpenGL::Ray.new(two3.clone, one3.clone)
    assert_equal(two3, a.origin)
    assert_equal(one3, a.direction)
  end

  def test_copy_equals
    a = OpenGL::Ray.new(zero3.clone, one3.clone)
    b = OpenGL::Ray.new.copy(a)
  ...
  def test_at
    a = OpenGL::Ray.new(one3.clone, OpenGL::Vector3.new(0, 0, 1))

    assert_equal(one3, a.at(0))
    assert_equal(OpenGL::Vector3.new(1, 1, 0), a.at(-1))
    assert_equal(OpenGL::Vector3.new(1, 1, 2), a.at(1))
  end
  ...
  def test_recast_clone
    ...
    b = a.clone
    assert_equal(OpenGL::Ray.new(OpenGL::Vector3.new(1, 1, 0), OpenGL::Vector3.new(0, 0, 1)), b.recast(-1))

    c = a.clone
    assert_equal(OpenGL::Ray.new(OpenGL::Vector3.new(1, 1, 2), OpenGL::Vector3.new(0, 0, 1)), c.recast(1))
  ...
  def test_closest_point_to_point
    ...
    # front of the ray
    c = a.closest_point_to_point(OpenGL::Vector3.new(0, 0, 50))
    assert_equal(OpenGL::Vector3.new(1, 1, 50), c)
  ...
  def test_distance_to_point
    ...
    # front of the ray
    c = a.distance_to_point(OpenGL::Vector3.new(0, 0, 50))
  ...
  def test_is_intersection_sphere
    a = OpenGL::Ray.new(one3.clone, OpenGL::Vector3.new(0, 0, 1))
    b = OpenGL::Sphere.new(zero3, 0.5)
    c = OpenGL::Sphere.new(zero3, 1.5)
    d = OpenGL::Sphere.new(one3, 0.1)
    e = OpenGL::Sphere.new(two3, 0.1)
    f = OpenGL::Sphere.new(two3, 1)
  ...
  def test_intersect_sphere
    ...
    a0 = OpenGL::Ray.new(zero3.clone, OpenGL::Vector3.new(0, 0, -1))
    # ray a1 origin located at (1, 1, 1) and points left in negative-x direction
    a1 = OpenGL::Ray.new(one3.clone, OpenGL::Vector3.new(-1, 0, 0))

    # sphere (radius of 2) located behind ray a0, should result in nil
    b = OpenGL::Sphere.new(OpenGL::Vector3.new(0, 0, 3), 2)
    ...
    b = OpenGL::Sphere.new(OpenGL::Vector3.new(3, 0, -1), 2)
    assert_equal(nil, a0.intersect_sphere(b))

    # sphere (radius of 2) located below ray a1, should result in nil
    b = OpenGL::Sphere.new(OpenGL::Vector3.new(1, -2, 1), 2)
    ...
    b = OpenGL::Sphere.new(OpenGL::Vector3.new(2.01, 0, -1), 2)
    assert_equal(nil, a0.intersect_sphere(b))
    ...
    b = OpenGL::Sphere.new(zero3.clone, 1)
    intersect = a0.intersect_sphere(b)
    unless intersect.nil?
      assert_in_delta(0, intersect.distance_to(OpenGL::Vector3.new(0, 0, -1)), TOL)
    else
    ...
    # thus keeping the intersection point always in front of the ray.
    b = OpenGL::Sphere.new(OpenGL::Vector3.new(0, 0, 1), 4)
    intersect = a0.intersect_sphere(b)
    unless intersect.nil?
      assert_in_delta(0, intersect.distance_to(OpenGL::Vector3.new(0, 0, -3)), TOL)
    else
      assert false, "Intersect was nil"
    end
    ...
    intersect = a0.intersect_sphere(b)
    unless intersect.nil?
      assert_in_delta(0, intersect.distance_to(OpenGL::Vector3.new(0, 0, -5)), TOL)
    else
      assert false, "Intersect was nil"
    end
  ...
  def test_is_intersection_plane
    ...
    b = OpenGL::Plane.new.set_from_normal_and_coplanar_point(OpenGL::Vector3.new(0, 0, 1), one3.clone.sub(OpenGL::Vector3.new(0, 0, -1)))
    assert(a.intersection_plane?(b))

    # parallel plane coincident with origin
    c = OpenGL::Plane.new.set_from_normal_and_coplanar_point(OpenGL::Vector3.new(0, 0, 1), one3.clone.sub(OpenGL::Vector3.new(0, 0, 0)))
    assert(a.intersection_plane?(c))

    # parallel plane behind the ray
    d = OpenGL::Plane.new.set_from_normal_and_coplanar_point(OpenGL::Vector3.new(0, 0, 1), one3.clone.sub(OpenGL::Vector3.new(0, 0, 1)))
    refute(a.intersection_plane?(d))

    # perpendical ray that overlaps exactly
    e = OpenGL::Plane.new.set_from_normal_and_coplanar_point(OpenGL::Vector3.new(1, 0, 0), one3)
    assert(a.intersection_plane?(e))

    # perpendical ray that doesn't overlap
    f = OpenGL::Plane.new.set_from_normal_and_coplanar_point(OpenGL::Vector3.new(1, 0, 0), zero3)
  ...
  def test_intersect_plane
    ...
    b = OpenGL::Plane.new.set_from_normal_and_coplanar_point(OpenGL::Vector3.new(0, 0, 1), OpenGL::Vector3.new(1, 1, -1))
    assert_equal(nil, a.intersect_plane(b))

    # parallel plane coincident with origin
    c = OpenGL::Plane.new.set_from_normal_and_coplanar_point(OpenGL::Vector3.new(0, 0, 1), OpenGL::Vector3.new(1, 1, 0))
    assert_equal(nil, a.intersect_plane(c))

    # parallel plane infront
    d = OpenGL::Plane.new.set_from_normal_and_coplanar_point(OpenGL::Vector3.new(0, 0, 1), OpenGL::Vector3.new(1, 1, 1))
  ...
  def test_apply_matrix4
    ...
    a = OpenGL::Ray.new(zero3.clone, OpenGL::Vector3.new(0, 0, 1))
    ...
    a.origin = OpenGL::Vector3.new(0, 0, 1)
    b.origin = OpenGL::Vector3.new(0, 0, -1)
    a2 = a.clone.apply_matrix4(m)
  ...
  def test_distance_sq_to_segment
    a = OpenGL::Ray.new(one3.clone, OpenGL::Vector3.new(0, 0, 1))
    ptOnLine = OpenGL::Vector3.new
    ptOnSegment = OpenGL::Vector3.new

    #segment in front of the ray
    v0 = OpenGL::Vector3.new(3, 5, 50)
    v1 = OpenGL::Vector3.new(50, 50, 50) # just a far away point
    distSqr = a.distance_sq_to_segment(v0, v1, ptOnLine, ptOnSegment)

    assert_in_delta(0, ptOnSegment.distance_to(v0), TOL)
    assert_in_delta(0, ptOnLine.distance_to(OpenGL::Vector3.new(1, 1, 50)), TOL)
    # ((3-1) * (3-1) + (5-1) * (5-1) = 4 + 16 = 20
    assert_in_delta(20, distSqr, TOL)

    #segment behind the ray
    v0 = OpenGL::Vector3.new(-50, -50, -50) # just a far away point
    v1 = OpenGL::Vector3.new(-3, -5, -4)
    distSqr = a.distance_sq_to_segment(v0, v1, ptOnLine, ptOnSegment)
    ...
    v0 = OpenGL::Vector3.new(-50, -50, -50)
    v1 = OpenGL::Vector3.new(50, 50, 50)
    distSqr = a.distance_sq_to_segment(v0, v1, ptOnLine, ptOnSegment)
  ...
  def test_intersect_box
    box = OpenGL::Box3.new(OpenGL::Vector3.new(-1, -1, -1), OpenGL::Vector3.new(1, 1, 1))

    a = OpenGL::Ray.new(OpenGL::Vector3.new(-2, 0, 0), OpenGL::Vector3.new(1, 0, 0))
    #ray should intersect box at -1,0,0
    assert a.intersection_box?(box)
    intersect = a.intersect_box(box)
    unless intersect.nil?
      assert_in_delta(intersect.distance_to(OpenGL::Vector3.new(-1, 0, 0)), TOL)
    else
      assert false, "Intersect was nil"
    end

    b = OpenGL::Ray.new(OpenGL::Vector3.new(-2, 0, 0), OpenGL::Vector3.new(-1, 0, 0))
    #ray is point away from box, it should not intersect
    refute b.intersection_box?(box)
    assert_nil b.intersect_box(box)

    c = OpenGL::Ray.new(OpenGL::Vector3.new(0, 0, 0), OpenGL::Vector3.new(1, 0, 0))
    ...
    d = OpenGL::Ray.new(OpenGL::Vector3.new(0, 2, 1), OpenGL::Vector3.new(0, -1, -1).normalize)
    #tilted ray should intersect box at 0,1,0
    assert_equal(true, d.intersection_box?(box))
    intersect = d.intersect_box(box)
    unless intersect.nil?
      assert_in_delta(intersect.distance_to(OpenGL::Vector3.new(0, 1, 0)), TOL)
    else
      assert false, "Intersect was nil"
    end

    e = OpenGL::Ray.new(OpenGL::Vector3.new(1, -2, 1), OpenGL::Vector3.new(0, 1, 0).normalize)
    ...
    unless intersect.nil?
      assert_in_delta(intersect.distance_to(OpenGL::Vector3.new(1, -1, 1)), TOL)
    else
      assert false, "Intersect was nil"
    end

    f = OpenGL::Ray.new(OpenGL::Vector3.new(1, -2, 0), OpenGL::Vector3.new(0, -1, 0).normalize)

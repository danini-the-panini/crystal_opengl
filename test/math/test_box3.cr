...
class TestBox3 < Minitest::Test
  ...
  def test_constructor
    ...
    a = OpenGL::Box3.new(zero3.clone, zero3.clone)
    assert_equal(zero3, a.min)
    assert_equal(zero3, a.max)

    a = OpenGL::Box3.new(zero3.clone, one3.clone)
    assert_equal(zero3, a.min)
  ...
  def test_set_from_points
    ...
    a.set_from_points([] of OpenGL::Vector3)
    assert(a.empty?)
  end
  ...
  def test_contains_box
    a = OpenGL::Box3.new(zero3.clone, zero3.clone)
    b = OpenGL::Box3.new(zero3.clone, one3.clone)
    c = OpenGL::Box3.new(one3.clone.negate, one3.clone)
  ...
  def test_get_parameter
    a = OpenGL::Box3.new(zero3.clone, one3.clone)
    b = OpenGL::Box3.new(one3.clone.negate, one3.clone)

    assert_equal(OpenGL::Vector3.new(0, 0, 0), a.parameter(OpenGL::Vector3.new(0, 0, 0)))
    assert_equal(OpenGL::Vector3.new(1, 1, 1), a.parameter(OpenGL::Vector3.new(1, 1, 1)))

    assert_equal(OpenGL::Vector3.new(0, 0, 0), b.parameter(OpenGL::Vector3.new(-1, -1, -1)))
    assert_equal(OpenGL::Vector3.new(0.5, 0.5, 0.5), b.parameter(OpenGL::Vector3.new(0, 0, 0)))
    assert_equal(OpenGL::Vector3.new(1, 1, 1), b.parameter(OpenGL::Vector3.new(1, 1, 1)))
  end
  ...
  def test_clamp_point
    ...
    assert_equal(OpenGL::Vector3.new(0, 0, 0), a.clamp_point(OpenGL::Vector3.new(0, 0, 0)))
    assert_equal(OpenGL::Vector3.new(0, 0, 0), a.clamp_point(OpenGL::Vector3.new(1, 1, 1)))
    assert_equal(OpenGL::Vector3.new(0, 0, 0), a.clamp_point(OpenGL::Vector3.new(-1, -1, -1)))

    assert_equal(OpenGL::Vector3.new(1, 1, 1), b.clamp_point(OpenGL::Vector3.new(2, 2, 2)))
    assert_equal(OpenGL::Vector3.new(1, 1, 1), b.clamp_point(OpenGL::Vector3.new(1, 1, 1)))
    assert_equal(OpenGL::Vector3.new(0, 0, 0), b.clamp_point(OpenGL::Vector3.new(0, 0, 0)))
    assert_equal(OpenGL::Vector3.new(-1, -1, -1), b.clamp_point(OpenGL::Vector3.new(-1, -1, -1)))
    assert_equal(OpenGL::Vector3.new(-1, -1, -1), b.clamp_point(OpenGL::Vector3.new(-2, -2, -2)))
  end
  ...
  def test_distance_to_point
    ...
    assert_equal(0, a.distance_to_point(OpenGL::Vector3.new(0, 0, 0)))
    assert_in_delta(Math.sqrt(3), a.distance_to_point(OpenGL::Vector3.new(1, 1, 1)))
    assert_in_delta(Math.sqrt(3), a.distance_to_point(OpenGL::Vector3.new(-1, -1, -1)))

    assert_in_delta(Math.sqrt(3), b.distance_to_point(OpenGL::Vector3.new(2, 2, 2)))
    assert_equal(0, b.distance_to_point(OpenGL::Vector3.new(1, 1, 1)))
    assert_equal(0, b.distance_to_point(OpenGL::Vector3.new(0, 0, 0)))
    assert_equal(0, b.distance_to_point(OpenGL::Vector3.new(-1, -1, -1)))
    assert_in_delta(Math.sqrt(3), b.distance_to_point(OpenGL::Vector3.new(-2, -2, -2)))
  end
  ...
  def test_is_intersection_box
    ...
    b.translate(OpenGL::Vector3.new(2, 2, 2))
  ...
  def test_get_bounding_sphere
    ...
    assert_equal(OpenGL::Sphere.new(zero3, 0), a.bounding_sphere)
    assert_equal(OpenGL::Sphere.new(one3.clone.multiply_scalar(0.5), Math.sqrt(3) * 0.5), b.bounding_sphere)
    assert_equal(OpenGL::Sphere.new(zero3, Math.sqrt(12) * 0.5), c.bounding_sphere)
  end
  ...
  def test_apply_matrix4
    ...
    b = OpenGL::Box3.new(zero3.clone, one3.clone)
    c = OpenGL::Box3.new(one3.clone.negate, one3.clone)
    d = OpenGL::Box3.new(one3.clone.negate, zero3.clone)

    m = OpenGL::Matrix4.new.make_translation(1, -2, 1)
    t1 = OpenGL::Vector3.new(1, -2, 1)
  ...
  def test_translate
    ...
    assert_equal(OpenGL::Box3.new(one3, one3), a.clone.translate(one3))

...
class TestSphere < Minitest::Test
  ...
  def test_constructor
    ...
    a = OpenGL::Sphere.new(one3.clone, 1)
  ...
  def test_intersects_sphere
    ...
    c = OpenGL::Sphere.new(zero3.clone, 0.25)
  ...
  def test_clamp_point
    ...
    assert_equal(OpenGL::Vector3.new(1, 1, 2), a.clamp_point(OpenGL::Vector3.new(1, 1, 3)))
    assert_equal(OpenGL::Vector3.new(1, 1, 0), a.clamp_point(OpenGL::Vector3.new(1, 1, -3)))
  end

  def test_get_bounding_box
    a = OpenGL::Sphere.new(one3.clone, 1)

    assert_equal(OpenGL::Box3.new(zero3, two3), a.bounding_box)

    a.set(zero3, 0)
    assert_equal(OpenGL::Box3.new(zero3, zero3), a.bounding_box)
  end

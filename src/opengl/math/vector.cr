...
module OpenGL
  class Vector
    ...
    def []=(index, value)
      ...
      raise IndexError.new if index.nil? || index < 0 || index >= self.class.dimensions
    ...
    def clamp(min, max)
      each_dimension do |i|
        @elements[i] = ExtraMath.clamp(@elements[i], min.elements[i], max.elements[i])
      end
      self
    end

    def clamp_scalar(min, max)
      @elements.map!{ |e| ExtraMath.clamp(e, min, max) }
      self
    end

    def floor
      @elements.map!{ |e| e.floor.to_f32 }
      self
    end

    def ceil
      @elements.map!{ |e| e.ceil.to_f32 }
      self
    end

    def round
      @elements.map!{ |e| e.round.to_f32 }
      self
    end

    def round_to_zero
      @elements.map!{ |e| (e < 0) ? e.ceil.to_f32 : e.floor.to_f32 }
      self
    end
    ...
    def angle_to(v)
      ...
      Math.acos(ExtraMath.clamp(theta, -1.0, 1.0))
    end
    ...
    def from_array(array : Array(Float32), offset = 0)
      each_dimension do |i|
    ...
    def to_array(array = [0f32, 0f32], offset = 0)
      each_dimension do |i|
    ...
    def to_a(array = [0f32, 0f32], offset = 0)

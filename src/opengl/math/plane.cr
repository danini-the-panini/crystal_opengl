...
module OpenGL
  class Plane
    property :normal, :constant

    @normal : Vector3
    @constant : Float32

    def initialize(@normal = Vector3.new(1, 0, 0), constant = 0)
      @constant = constant.to_f32
    end
    ...
    def project_point(point, target = OpenGL::Vector3.new)
      self.ortho_point(point, target).sub(point).negate
    end

    def ortho_point(point, target = OpenGL::Vector3.new)
    ...
    def intersect_line(line, target = OpenGL::Vector3.new)
      v1 = OpenGL::Vector3.new
      direction = line.delta(v1)
      denominator = @normal.dot(direction)
      if denominator == 0
        # line is coplanar, return origin
        if self.distance_to_point(line.start_point) == 0
    ...
    def coplanar_point(target = OpenGL::Vector3.new)
      target.copy(@normal).multiply_scalar(- @constant)
    end

    def apply_matrix4(matrix, normal_matrix = OpenGL::Matrix3.new.normal_matrix(matrix))
      v1 = OpenGL::Vector3.new
      v2 = OpenGL::Vector3.new

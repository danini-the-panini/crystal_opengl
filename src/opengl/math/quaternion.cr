module OpenGL
  class Quaternion
    ...
    def initialize(x = 0, y = 0, z = 0, w = 1)
      @x, @y, @z, @w = x.to_f32, y.to_f32, z.to_f32, w.to_f32
    end
    ...
    def set_from_axis_angle(axis, angle)
      ...
      half_angle = angle / 2.0f32
      s = Math.sin(half_angle)
      @x = axis.x * s
      @y = axis.y * s
      @z = axis.z * s
      @w = Math.cos(half_angle).to_f32
      self.on_change_callback
      self
    end
    ...
    def set_from_rotation_matrix(m)
      ...
      trace = m11 + m22 + m33
      if trace > 0
        s = 0.5f32 / Math.sqrt(trace + 1.0f32)
        @w = 0.25f32 / s
        @x = (m32 - m23) * s
        @y = (m13 - m31) * s
        @z = (m21 - m12) * s
      elsif m11 > m22 && m11 > m33
        s = 2.0f32 * Math.sqrt(1.0 + m11 - m22 - m33)
        @w = (m32 - m23) / s
        @x = 0.25f32 * s
        @y = (m12 + m21) / s
        @z = (m13 + m31) / s
      elsif m22 > m33
        s = 2.0f32 * Math.sqrt(1.0 + m22 - m11 - m33)
        @w = (m13 - m31) / s
        @x = (m12 + m21) / s
        @y = 0.25f32 * s
        @z = (m23 + m32) / s
      else
        s = 2.0f32 * Math.sqrt(1.0 + m33 - m11 - m22)
        @w = (m21 - m12) / s
        @x = (m13 + m31) / s
        @y = (m23 + m32) / s
        @z = 0.25f32 * s
    ...
    def slerp(qb, t)
      ...
      sin_half_theta = Math.sqrt(1.0f32 - cos_half_theta * cos_half_theta)
      ...
      ratio_a = Math.sin((1.0f32 - t) * half_theta) / sin_half_theta
    ...
    def to_array(array = [] of Float32, offset = 0)
      array[offset] = @x
      array[offset + 1] = @y
      array[offset + 2] = @z
      array[offset + 3] = @w

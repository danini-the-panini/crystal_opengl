module OpenGL
  class Ray
    ...
    def initialize(origin = OpenGL::Vector3.new, direction = OpenGL::Vector3.new)
    ...
    def closest_point_to_point(point, target = OpenGL::Vector3.new)
    ...
    def distance_sq_to_segment(v0, v1, point_on_ray = nil, point_on_segment = nil)
      seg_center = OpenGL::Vector3.new
      seg_dir = OpenGL::Vector3.new
      diff = OpenGL::Vector3.new
    ...
    def intersect_sphere(sphere, target = OpenGL::Vector3.new)
    ...
    def intersection_plane?(plane)
      ...
      return true if dist_to_point == 0
    ...
    def distance_to_plane(plane)
      ...
      if denominator == 0
        ...
        return 0.0 if plane.distance_to_point(@origin) == 0
    ...
    def intersect_plane(plane, target = OpenGL::Vector3.new)
    ...
    def intersection_box?(box)
      v = OpenGL::Vector3.new
      !self.intersect_box(box, v).nil?
    end

    def intersect_box(box, target = OpenGL::Vector3.new)
      # http:#www.scratchapixel.com/lessons/3d-basic-lessons/lesson-7-intersecting-simple-shapes/ray-box-intersection/
      invdirx = 1.0f32 / @direction.x
      invdiry = 1.0f32 / @direction.y
      invdirz = 1.0f32 / @direction.z
      origin = @origin
      if invdirx >= 0
      ...
      tmin = tymin if tymin > tmin || tmin.nan?
      tmax = tymax if tymax < tmax || tmax.nan?
      if invdirz >= 0
      ...
      tmin = tzmin if tzmin > tmin || tmin.nan?
      tmax = tzmax if tzmax < tmax || tmax.nan?
      #return point closest to the ray (positive side)
      return nil if tmax < 0
      self.at(tmin >= 0 ? tmin : tmax, target)
    end

    def intersect_triangle(a, b, c, backface_culling, target = OpenGL::Vector3.new)
      # Compute the offset origin, edges, and normal.
      diff = OpenGL::Vector3.new
      edge1 = OpenGL::Vector3.new
      edge2 = OpenGL::Vector3.new
      normal = OpenGL::Vector3.new

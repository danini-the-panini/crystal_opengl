module OpenGL
  class Spline
    ...
    def length(n_sub_divisions = 100)
      ...
      old_position = OpenGL::Vector3.new
      tmp_vec = OpenGL::Vector3.new
      chunk_lengths = [] of Int32
      total_length = 0
      ...
      (1...n_samples).each do |i|
        index = i.to_f32 / n_samples.to_f32
    ...
    def reparametrize_by_arc_length(sampling_coef)
      ...
      @points.each_with_index do |p,i|
        ...
        index_current = (i.to_f32 - 1.0) / (@points.length.to_f32 - 1.0)
        index_next = i.to_f32 / (@points.length.to_f32 - 1.0)
    ...
    private def interpolate(p0, p1, p2, p3, t, t2, t3)
      v0 = (p2 - p0) * 0.5
      v1 = (p4 - p1) * 0.5

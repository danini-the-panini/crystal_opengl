...
module OpenGL
  class Box3
    ...
    def initialize(min = nil, max = nil)
      @min = min || Vector3.new(Float32::INFINITY, Float32::INFINITY, Float32::INFINITY)
      @max = max || Vector3.new(-Float32::INFINITY, -Float32::INFINITY, -Float32::INFINITY)
    end
    ...
    def set_from_object(object)
      ...
      object.traverse do |node|
        ...
        if geometry != nil
          ...
          elsif geometry instanceof BufferGeometry && geometry.attributes["position"] != nil
    ...
    def make_empty
      @min.x = @min.y = @min.z = Float32::INFINITY
      @max.x = @max.y = @max.z = -Float32::INFINITY
      self
    end
    ...
    def center(target = Vector3.new)
      target.add_vectors(@min, @max).multiply_scalar(0.5)
    end

    def size(target = Vector3.new)
    ...
    def distance_to_point(point, target = Vector3.new)
      target.copy(point).clamp(@min, @max).sub(point).length
    end

    def bounding_sphere(target = Sphere.new)
    ...
    def apply_matrix4(matrix)
      points = [
        ...
        Vector3.new(@min.x, @min.y, @min.z).apply_matrix4(matrix), # 000
        Vector3.new(@min.x, @min.y, @max.z).apply_matrix4(matrix), # 001
        Vector3.new(@min.x, @max.y, @min.z).apply_matrix4(matrix), # 010
        Vector3.new(@min.x, @max.y, @max.z).apply_matrix4(matrix), # 011
        Vector3.new(@max.x, @min.y, @min.z).apply_matrix4(matrix), # 100
        Vector3.new(@max.x, @min.y, @max.z).apply_matrix4(matrix), # 101
        Vector3.new(@max.x, @max.y, @min.z).apply_matrix4(matrix), # 110
        Vector3.new(@max.x, @max.y, @max.z).apply_matrix4(matrix)  # 111
      ]
      self.make_empty
